...
import TournamentCountdown from "../tournament-countdown/TournamentCountdown";
...
export default class TournamentList extends Component {
  ...
  filterByNickName = evt => {
    ...
    const filteredTournaments = this.props.tournaments.filter(tournament => {
      ...
      const expandedTournament = {
        ...tournament,
        results: tournament.results.map(
          resultId => this.props.results.find(result => result.id === resultId),
          this
        )
      };
      ...
      return expandedTournament.results.some(
        result =>
  ...
  render() {
    const tournamentItems = this.state.tournaments.map(
      (tournament, i) => (
        <Tournament
          key={`tournament_${i}`}
